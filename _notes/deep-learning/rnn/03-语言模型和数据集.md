...
permalink: /notes/language-models-and-datasets/
redirect_from:
  - /notes/RNN语言模型和数据集/
---
...
1. **每个 epoch 打乱语料顺序**：增加样本多样性
1. **随机选择句子起点**：保证语义完整性
1. **使用掩码语言建模**：如 BERT 的 MLM 任务

### 8. 如果序列样本是完整句子，会有什么问题？如何解决？
...
1. **Bucketing（分桶）**：按句长分桶，相近长度的句子组成一批
1. **Dynamic Batching（动态批次）**：动态调整 batch 大小
1. **Packed Sequence（打包序列）**：如 PyTorch 的 `pack_padded_sequence`，高效处理变长序列
