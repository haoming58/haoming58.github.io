...
在开始编写模型代码之前，我们需要明确输入数据的形状以及 GRU 层中各个权重和偏置参数的维度。
...
**函数说明：**
- 输入：`inputs` - 时间步数据，`state` - 初始隐藏状态，`params` - 模型参数
- 循环处理：每次提取一个时间步的数据 `X`，其形状为 `(batch_size, vocab_size)`
- `@` 运算符：PyTorch（以及 NumPy 3.5+）中专门用于执行矩阵乘法
- `torch.cat(..., dim=0)`：沿着第 0 维（行）将这些张量拼接起来
...
| 超参数 | 参数名 | 示例值 | 说明 |
|--------|--------|--------|------|
| 隐藏层单元数 | `num_hiddens` | 256, 32 等 | 决定"脑容量"。越大能记住越复杂的信息，但更容易过拟合且计算更慢。 |
