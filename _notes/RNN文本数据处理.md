...
**说明**：
- `collections`：提供了很多方便的数据结构，比如：
  - `Counter`：可以用来统计词频，非常适合做文本数据处理
  - `defaultdict`：带默认值的字典
- `re`：用于文本清洗、模式匹配、替换等操作
- `d2l`：深度学习工具库
...
**说明**：
- `re.sub('[^A-Za-z]+', ' ', line)` → 只保留英文字母，其他字符替换为空格
- `.strip()` → 去掉行首尾空格
- `.lower()` → 全部转换为小写
...
将文本行转换为最小的基本单位——**词元（token）**。词元可以是单词（WORD）或字符（W O R D）。
...
**说明**：
- `token='word'`：按单词拆分
  - `line.split()` 将字符串按空白字符拆分
  - 示例：`"Hello world Python"` → `['Hello', 'world', 'Python']`
- `token='char'`：按字符拆分
  - `list(line)` 将字符串拆分成单个字符
  - 示例：`"Hello"` → `['H', 'e', 'l', 'l', 'o']`
