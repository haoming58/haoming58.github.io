...
写4 个 for 循环: 可以把 for 循环当作是

```python
class Solution:
    def find_tuple(self, a: list[int], b: list[int], c: list[int], d: list[int]) -> list[tuple[int, int, int, int]]:
        result = []
...
把 4 个数组分成两组，每组算和，然后两组的和相加为 0。

利用哈希表存数据，然后使用哈希表去找数据，并且删除数据,

这里的哈希表应该是允许重复的，至于排序应该不用考虑，

不同下标可能得到相同的和, 因此，set，不可以使用。

涉及到重复结果，次数检查的时候，counter， key，value
...
class solution():
    def find_tuple(self, a: list[int], b: list[int], c: list[int], d: list[int]) -> list[tuple[int, int, int, int]]:
        ...
        '''
        hashmap.get(n1+n2, 0)
        如果 n1 + n2 已经是字典的 key 返回对应的 value
        如果 还不存在 返回 0
        然后
        + 1
        统计 nums1 和 nums2 所有可能的和，及其出现次数
...
这个赎金信，实际上就是考虑交集的事情，意思是 赎金信 是 杂志的子集。

思路一，数组，下标是字母，数值是次数，最后只要保证杂志的每个数值是大于等于赎金信
```
class Solution:
...
思路二，采用字典，利用杂志的数据创建一个字典，然后循环判断赎金信的是否在就行，只要其中一个不在就判断失败。


```python

这个就是智能字典，就是这个 defaultdict，没有的话，就能直接返回None.
...
# 对每个字符做 ransomNote[char] - magazine[char]
...
# 如果赎金信中所需的每个字符，在杂志中都能找到足够多的数量，就返回 True；否则返回 False
...
最简单的方法，暴力循环法.以及这个和之前的4数相加是一样的.

“三数之和”的核心在于利用排序将无序的搜索转化为有序的组合问题。排序后，我们通过固定第一个数并使用双指针左右夹击，将复杂度从 $O(N^3)$ 降维至 $O(N^2)$；此时去重变得极简——只需在外层遍历（固定老大）和内层收缩（双指针移动）时，利用数据的单调性判断“当前元素是否与上一个相同”，若相同则直接跳过，这样既保留了不同下标但数值相同的合法组合，又在源头上彻底根除了重复结果，无需任何额外的哈希表或集合空间。
