...
这个就是智能字典，就是这个 defaultdict，没有的话，就能直接返回 None。
...
但是，基于视频和文字，我没有过多的思考，而且重点是涉及到去重，因此，直接采用数值的双指针方法。三原组的不能重复，但是本身的数据是可以重复的。意思结果集合里面可以有元素。

先进行排序，

这道题目，实际上是组合，因为是集合，顺序是不用考虑的，针对for循环之类的，就需要对于下标进行克制，不用全部枚举。
